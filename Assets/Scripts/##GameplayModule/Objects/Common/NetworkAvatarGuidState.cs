...
namespace Unity.Assets.Scripts.Objects
{
    ...
    public class NetworkAvatarGuidState : NetworkBehaviour
    {
        ...
        /// 현재 등록된 아바타 인스턴스입니다.
        /// </summary>
        MonsterAvatarSO m_MonsterAvatar;

        /// <summary>
        /// 현재 등록된 아바타를 반환하는 프로퍼티입니다.
        /// 아바타가 등록되지 않은 경우 자동으로 등록을 시도합니다.
        /// </summary>
        /// <remarks>
        /// 디버깅: 이 프로퍼티 접근 시 m_Avatar가 null이면 RegisterAvatar가 호출됩니다.
        /// </remarks>
        public MonsterAvatarSO RegisteredMonsterAvatar
        {
            get
            {
                if (m_MonsterAvatar == null)
                {
                    // DEBUG: AvatarGuid.Value가 유효한지 로그로 확인
                    Debug.Log($"Registering avatar with GUID: {AvatarGuid.Value.ToGuid()}");
                    RegisterAvatar(AvatarGuid.Value.ToGuid());
                }

                return m_MonsterAvatar;
            }
        }

        /// <summary>
        /// 레지스트리에서 무작위 아바타를 선택하여 설정합니다.
        /// </summary>
        /// <remarks>
        /// 디버깅: 이 메서드 호출 시 m_AvatarRegistry가 null이 아닌지 확인하세요.
        /// </remarks>
