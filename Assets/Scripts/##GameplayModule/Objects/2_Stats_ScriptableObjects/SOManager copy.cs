    ...
    public class SSManager
    {
        ...
        public void Initialize()
        {
            ...
            if (resourceManager != null)
            {
                ...
                // 이미 리소스가 로드되었을 수 있으므로 확인
                // 이 부분은 리소스 매니저에 리소스가 이미 로드되었는지 확인하는 메서드가 있다면 사용
                // 예: if (resourceManager.IsLoaded) { ... }
            }
            else
            {
        ...
        private void LoadAllMonsterAvatarSO()
        {
            ...
            foreach (var asset in assets)
            {
                ...
                if (asset.MonsterData != null)
                {
                    ...
                    // 3. 에셋 경로 설정 (아직 설정되지 않은 경우에만)
                    if (string.IsNullOrEmpty(asset.AnimatorControllerPath))
                    {
                        string monsterKey = GetMonsterKeyFromCharacterType(asset.MonsterData.CharacterType);
                        if (!string.IsNullOrEmpty(asset.MonsterData.PrefabLabel))
                        {
                            monsterKey = asset.MonsterData.PrefabLabel;
                        }
                        
                        if (!string.IsNullOrEmpty(monsterKey))
                        {
                            asset.SetAnimatorControllerPath($"Assets/01_Animations/{monsterKey}/{monsterKey}");
                            asset.SetSpritePath($"Assets/Package/Character/{monsterKey}");
                            asset.SetPrefabPath($"Assets/02_Prefabs/Creature_Prefab/{monsterKey}");
                            Debug.Log($"[SOManager] {asset.name}의 에셋 경로 설정 완료");
                        }
                    }
                }
                else
        ...
        private void LoadSprite(MonsterAvatarSO avatarSO, string prefabLabel)
        {
            ...
            foreach (var path in possiblePaths)
            {
                ...
                // .sprite 접미사 추가 시도
                foundSprite = resourceManager.Load<Sprite>($"{path}.sprite");
                if (foundSprite != null)
                {
