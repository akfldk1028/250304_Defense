...
class ClientConnectingState : OnlineState
{
    ...
    //         {
    //             Debug.LogWarning("[ClientConnectingState] 연결 타임아웃 이후 OnClientConnected 호출됨 - 무시");
    //             return;
    //         }
    ...
    //         // 상태 전환 후 추가 체크 - 플레이어 수 확인 및 씬 전환
    //         if (m_LocalLobby.LobbyUsers.Count >= m_ConnectionManager.MaxConnectedPlayers)
    //         {
    //             Debug.Log("[ClientConnectingState] 플레이어 수 충족 - 게임 씬으로 전환");
    //             _sceneManagerEx.LoadScene(EScene.BasicGame.ToString(), useNetworkSceneManager: true);
    //         }
    //     }
    //     catch (Exception e)
